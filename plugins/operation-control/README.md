...
`register-goal` → `propose-milestone` → `confirm-milestone` → `create-root-task` → `create-child-task` → `review-node` → `review-milestone`.
...
Agents receive `propose-milestone`, `create-root-task`, `create-child-task`, and `review-node` tools. Keep worker Agents' normal task-assignment permission disabled; these plugin tools are the narrow delivery path.
