...
export function OperationControlWidget({ context }: PluginWidgetProps) {
  ...
  useEffect(() => {
    ...
    const preferred = data.state.ownerAgentId
      ?? data.agents.find((agent) => /\b(?:maintainer|devops|sre)\b/i.test(`${agent.name} ${agent.title ?? ""}`))?.id
  ...
  return (
    <section style={{ display: "grid", gap: 12 }}>
      ...
      <small>
        Goal delivery: {data.autonomy.autoDispatchGoals ? "automatic" : "manual"} via {data.autonomy.orchestratorRole}; review and approval use Paperclip native execution policies.
      </small>
      {data.runBudget.limit > 0 && (
        <small>Hourly run budget: {data.runBudget.count} / {data.runBudget.limit}</small>
      )}
      {data.state.mode === "holding" && <div>{runningCount} agent(s) are finishing current work.</div>}
      ...
      {active ? (
        <button disabled={busy} onClick={() => void run(() => resumeNormal({ companyId: context.companyId }))}>
