...
5. Root Task가 끝나면 Product Steward가 Milestone 완료 근거를 모아 사람에게 최종 확인을 요청한다.
...
| 인간 Milestone 확인 | 별도 approval stage 또는 Milestone comment의 확인 기록 |
...
- Product Steward: Milestone 초안, Root Task 생성, 실행 Agent 배정, tree 진행 조정, 완료 근거 취합.
- Node 담당 Agent: child 분해, leaf 실행 확인, Node review 제출, 거절된 Node의 보완 child 생성.
- 실행 Agent: 할당된 leaf 또는 child 하나를 수행하고 evidence를 남긴다. Goal, Milestone 상태와 sibling 범위를 바꾸지 않는다.
