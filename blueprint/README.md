...
[Role 계약](roles.md)은 책임과 권한을, [Delivery Lifecycle](delivery-lifecycle.md)은 Task 표현과 Role 전환을 정한다.
...
2. Product Steward가 인간 요청을 기존 또는 신규 Team Goal Milestone에 정렬한다.
3. Product Steward가 제품 상태에 맞는 Delivery Role과 entry·exit gate를 정한다.
4. Prototyper는 여러 후보를 만들고 Product Steward는 keep 또는 kill을 결정한다.
5. Builder는 선택된 후보를 제품화하고 Task의 review stage를 거친다.
6. Grower는 사용자·eval 근거로 개선을 요청하고 Sweeper는 불필요한 복잡성을 제거한다.
7. Maintainer는 운영 가능한 시스템의 보안, 신뢰성, 성능과 비용을 책임진다.
8. Product Steward가 완료 근거로 Milestone 상태를 관리하고 주요 전환은 Board에 보고한다.
9. Maintainer가 필요하면 [Maintenance](../docs/architecture.md)를 요청하고 단일 owner로 실행한 뒤 `normal`로 복귀한다.
...
6. Company Goal 하나와 첫 `team` Goal Milestone, Project를 만든다.
7. workspace의 Git 기준점과 isolated workspace 지원 여부를 확인하고 안전한 실행 policy를 선택한다.
8. Operation Control의 owner를 Maintainer로 선택하고 `drain → maintenance → normal`을 시험한다.
9. 각 Role Task 하나가 배정, 실행, review, 전환되는지 검증한다.
