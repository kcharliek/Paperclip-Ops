...
Role 이름보다 [책임과 권한 계약](roles.md)이 우선이다. 작은 Company는 Builder와 Researcher를 한 Agent가 맡을 수 있지만 Manager와 Tech Manager의 실행 모드는 분리한다.
...
| Delivery | active Goal 하나, Project, Task 완료 조건 |
...
2. Manager가 Goal을 실행 가능한 Task로 나누고 Role에 배정한다.
3. Builder와 Researcher가 근거와 산출물을 Task에 남긴다.
4. Manager가 완료 조건을 검토하고 다음 Task를 연다.
5. Tech Manager가 주기적으로 부채를 확인하고 필요하면 [Maintenance](../docs/architecture.md)를 시작한다.
6. Maintenance에서는 Task 조정, 승인된 Agent 채용, 코드 개선과 검증을 마친 뒤 `normal`로 복귀한다.

## 설정 순서

1. Company와 Board를 만든다.
2. Manager와 Tech Manager를 만들고 보고선·권한을 검증한다.
3. 필요한 실행 Role만 추가한다.
4. Company Skill과 writable workspace를 연결한다.
5. Company Goal 하나와 첫 Project를 만든다.
6. Operation Control을 연결하고 `drain → maintenance → normal`을 시험한다.
7. 실제 Task 하나가 배정, 실행, review, 완료되는지 확인한다.
