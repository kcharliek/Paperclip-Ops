...
- Routine은 DB restore, `resume normal`, Agent·Role·권한·instruction, Goal·Milestone·Task tree, Routine 설정 또는 제품 코드를 직접 변경하지 않는다. Product Steward가 별도 Maintenance Task를 승인하면 Maintainer가 기존 Maintenance 절차로 보정하고 정상 복귀 뒤 독립 review를 받는다.
