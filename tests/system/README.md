...
- Board가 일회용 Company에서 종료된 orchestration Task를 누락 상태로 만들고 복구 action으로 새 Task를 생성한 뒤, 반복 복구가 같은 Task만 다시 깨우며 Goal·Project·origin 연결과 단일 pending 계약을 보존하는지 확인한다.
