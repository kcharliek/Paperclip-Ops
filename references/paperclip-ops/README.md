...
- Operation Control: `normal`, 시간당 run 20회, `milestone_pending`
- Routine: `System Improvement Review` 주 1회, `Company Integrity Check` 6시간마다

System Improvement Review의 첫 수동 실행 `PAP-1`은 성공했고, 현재 drift는 `PAP-2..13`, `PAP-15` Backlog가 추적한다. Ops Steward의 `PAP-14`는 `PAP-4`만 필수 범위로 둔 첫 Milestone 초안을 제출했고 사람 확인을 기다린다. 상세 ID와 남은 차이는 [current-state](current-state.md)와 [drift](drift.md)에 있다.
