...
- Alex와 FormDe Spec은 큰 Task를 자기 자신에게 할당한 child로만 분해할 수 있다. 다른 Agent에게 보내는 분배는 Charlie가 담당한다.
...
- Alex 또는 FormDe Spec이 담당하는 Root에는 Charlie 한 명을 review stage로 설정한다.
- Charlie가 담당하는 Goal 전환 Root에는 Board를 reviewer 또는 approver로 설정한다.
