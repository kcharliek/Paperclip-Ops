...
0.6.1 Board 복구 action의 기존 orchestration Task 재-wake는 plugin test로 확인했고, 현재 `milestone_pending` live 호출은 `human_gate` 무변경 결과였다. 따라서 복구 표면은 생겼지만 state 전이와 Issue 생성의 원자성 자체는 바뀌지 않았다.
